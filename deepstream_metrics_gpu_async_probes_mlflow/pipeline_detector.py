#!/usr/bin/env python3
"""
DeepStream 8 detector (Gst-Python + pyds).

  nvurisrcbin --vsrc_0--> nvstreammux --> nvinfer --> nvvideoconvert
       --> nvdsosd --> nvvideoconvert --> NV12 --> nvv4l2h264enc
       --> h264parse --> qtmux --> filesink

--log-mode async  pad probe only enqueue (default)
--log-mode probe  JSONL write+flush inside the OSD probe (A/B latency)
"""
...
DEFAULT_CONFIG = "/workspace/configs/pgie_config.txt"
DEFAULT_OUTPUT = "/workspace/output/detector_annotated.mp4"
...
def maybe_reexec_nsys() -> None:
    ...
    if os.environ.get("NSYS_ACTIVE") == "1":
        return
    out = f"/workspace/output/nsys/detector_{int(time.time())}"
...
def osd_sink_pad_probe(pad, info, user_data):
    ...
    while l_frame is not None:
        ...
        e2e_ms, hops = metrics.note_osd_frame(frame_meta.frame_num, n_obj, by_class)
        hops = metrics.hops_for_frame(frame_meta.frame_num) or hops
...
def build_pipeline(uri: str, output_path: str, config_path: str, metrics: StageMetrics):
    pipeline = Gst.Pipeline.new("ds8-file-detector")
    ...
    if not pgie.link(conv_osd):
        raise RuntimeError("link pgie -> conv_osd failed")
    ...
    if not all((mux_src, pgie_src, osd_sink, enc_sink)):
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="DS8 detector -> annotated MP4 + metrics")
    ...
    run_name = args.run_name or f"detector_{args.log_mode}_{int(time.time())}"
    ...
    mlflow_mod = None
    try:
        mlflow_mod, _run = start_run(run_name, params)
    ...
    pipeline, osd_sink = build_pipeline(uri, output_path, config_path, metrics)
    osd_sink.add_probe(
    ...
    artifacts = [config_path, metrics_json]
